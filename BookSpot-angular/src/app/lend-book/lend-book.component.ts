 ...
 userRecord: any[] = [];
  ...
  constructor(private cS:CategoryService,private datePipe: DatePipe,private fb:FormBuilder) {
    ...
    if(this.loc!=null){
      ...
      // this.To_book=this.details.To_book.name
      // this.To_cate=this.details.selectedCategory 
...
// console.log(cat_id.value);
  ...
  add(){      
    ...
    else if (Number(Days)==2||Number(Days)==3||Number(Days)==4|| Number(Days) == 5){
      this.result = 100;      
      return this.result;       
    }
...
//-- end save--
  ...
  submit(){
    ...
    if(this.lendForm.invalid){
      alert("The form is empty..Kindly fill the form..!")
    }
    else{    
    this.lendForm.value.price=this.result;
    this.lendForm.value.lcal=this.day;
